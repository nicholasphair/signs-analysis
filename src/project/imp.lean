...
def aeval : aexp → nat
| (aexp.ANum n) := n
| (aexp.APlus a b) := nat.add (aeval a) (aeval b)
| (aexp.AMinus a b) := nat.sub (aeval a) (aeval b)
| (aexp.AMult a b) := nat.mul (aeval a) (aeval b)

--  prove the proposition, aevel 5 = 5
example: aeval (aexp.ANum 5) = 5 := rfl
#check aeval (aexp.ANum 5) = 5  -- its a prop.


def beval: bexp -> bool
| bexp.BTrue := tt
| bexp.BFalse := ff
| (bexp.BEq a₁ a₂) := (aeval a₁) = (aeval a₂) -- interesting, there are lot of definitions of equality (refl, sym)
| (bexp.BNeq a₁ a₂) := ¬ (aeval a₁) = (aeval a₂) 
| (bexp.BLe a₁ a₂) := (aeval a₁) ≤ (aeval a₂) 
| (bexp.BGt a₁ a₂) := (aeval a₁) > (aeval a₂)
| (bexp.BNot b₁) := bnot (beval b₁) 
| (bexp.BAnd b₁ b₂) := band (beval b₁) (beval b₂)

-- #eval
example: beval (bexp.BLe (aexp.ANum 4) (aexp.ANum 5)) = tt := rfl
example: beval (bexp.BAnd bexp.BTrue bexp.BTrue) = tt := rfl
example: beval (bexp.BAnd bexp.BFalse bexp.BTrue) = ff := rfl
